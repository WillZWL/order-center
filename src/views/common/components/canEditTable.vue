...
<template>
    <div>
        <Table :size="size" :ref="refs" :columns="columnsList" :data="thisTableData" border disabled-hover></Table>
    </div>
</template>
...
const customeButton = (vm, h, currentRow, index, name) => {
    return h('Button', {
        props: {
            type: 'primary'
        },
        on: {
            'click': () => {
                vm.$emit('click-customButton', vm.handleBackdata(vm.thisTableData), index);
                vm.$emit('input', vm.handleBackdata(vm.thisTableData));
            }
        }
    }, name);
            ...
            this.columnsList.forEach(item => {
                ...
                if (item.handle) {
                    item.render = (h, param) => {
                        ...
                        if (item.handle.length === 3) {
                            if (item.handle[0] === '' && item.handle[1] === '') {
                                return h('div', [
                                    customeButton(this, h, currentRowData, param.index, this.customeButtonName)
                                ]);
                            }
                            if (item.handle[0] === '') {
                                return h('div', [
